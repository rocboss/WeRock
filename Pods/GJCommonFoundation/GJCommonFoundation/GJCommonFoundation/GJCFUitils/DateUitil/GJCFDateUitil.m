...
+ (NSString *)stringDaysAgoAgainstMidnight:(BOOL)flag withDate:(NSDate *)date {
    ...
    switch (daysAgo) {
        ...
        case 1:
            ...
            break;
        default:
            text = [NSString stringWithFormat:@"%d天前", (NSInteger)daysAgo];
    }
    return text;
}
...
+ (NSString *)weekNumberString:(NSDate *)date
{
    ...
    return [NSString stringWithFormat:@"第%d周",GJCFDateGetWeekNum(date)];
}
...
+ (NSString *)birthdayToAge:(NSDate *)date
{
    ...
    if ([components year] > 0) {
        ...
        return [NSString stringWithFormat:@"%d岁",[components year]];

    }else if([components month] > 0) {
        
        return [NSString stringWithFormat:@"%d个月%d天",[components month],[components day]];
        
    }else{
        
        return [NSString stringWithFormat:@"%d天",[components day]];
