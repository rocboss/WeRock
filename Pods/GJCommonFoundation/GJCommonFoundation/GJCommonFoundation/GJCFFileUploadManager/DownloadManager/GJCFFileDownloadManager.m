...
- (id)init
{
    if (self = [super init]) {
        ...
        self.httpClient = [[AFHTTPClient alloc]initWithBaseURL:[NSURL URLWithString:@"http://www.ganji.com"]];
    }
    return self;
}
...
- (void)addTask:(GJCFFileDownloadTask *)task
{
    ...
    dispatch_async(_gjcfFileDownloadManagerOperationQueue, ^{
        ...
        /* 加入队列 */
        if (self.httpClient) {
            [self.httpClient enqueueHTTPRequestOperation:downloadOperation];
...
- (void)cancelTask:(NSString *)taskUniqueIdentifier
{
    ...
    dispatch_async(_gjcfFileDownloadManagerOperationQueue, ^{
        ...
        /* 退出任务 */
        for (AFHTTPRequestOperation *request in self.httpClient.operationQueue.operations) {
