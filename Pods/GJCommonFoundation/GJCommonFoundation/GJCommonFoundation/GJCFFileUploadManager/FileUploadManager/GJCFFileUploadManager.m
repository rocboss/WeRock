...
@property (nonatomic,strong)AFHTTPClient *requestClient;
...
- (void)initClient
{
    ...
    self.requestClient = [[AFHTTPClient alloc]initWithBaseURL:[NSURL URLWithString:self.defaultHostUrl]];
    [self.requestClient registerHTTPOperationClass:[AFJSONRequestOperation class]];
    self.requestClient.stringEncoding = NSUTF8StringEncoding;
    self.requestClient.parameterEncoding = AFFormURLParameterEncoding;
    ...
    [self.defualtRequestHeader enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        [self.requestClient setDefaultHeader:key value:obj];
    }];
}

- (NSMutableURLRequest *)requestWithTask:(GJCFFileUploadTask*)aTask
{
    ...
    NSLog(@"defaultRequestParams:%@",self.defaultRequestParmas);
    
    NSMutableURLRequest *request = [self.requestClient multipartFormRequestWithMethod:@"POST" path:self.defaultPath parameters:self.defaultRequestParmas constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        for (GJCFUploadFileModel *aFile in fileModelArray) {
            ...
            if (![aFile isKindOfClass:[GJCFUploadFileModel class]]) {
                ...
                NSLog(@"GJCFUploadFile :%@",aFile.description);
    ...
    NSLog(@"defaultHeader:%@",self.defualtRequestHeader);
    [self.defualtRequestHeader enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        
        [request setValue:obj forHTTPHeaderField:key];
        
    }];
    NSLog(@"customHeader:%@",aTask.customRequestHeader);
...
- (void)addTask:(GJCFFileUploadTask *)aTask
{
    ...
    dispatch_async(_gjcfFileUploadManagerOperationQueue, ^{
        ...
        //开始实际上传
        NSMutableURLRequest *uploadRequest = [self requestWithTask:aTask];
        ...
        [self.requestClient enqueueHTTPRequestOperation:uploadOperation];
...
- (void)cancelTask:(NSString *)aTaskIdentifier shouldRemove:(BOOL)remove
{
    ...
    dispatch_async(_gjcfFileUploadManagerOperationQueue, ^{
        ...
        /* 推出请求 */
        [self.requestClient.operationQueue.operations enumerateObjectsUsingBlock:^(AFHTTPRequestOperation *operation, NSUInteger idx, BOOL *stop) {
